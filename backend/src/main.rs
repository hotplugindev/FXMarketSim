...
async fn place_trade(
    ...
    if let Some(broker) = brokers.get(&payload.broker_id) {
        match market.place_order(&payload.symbol, payload.side, payload.amount, broker.clone()).await {
            Ok(order_id) => {
...
async fn get_brokers(State(state): State<AppState>) -> impl IntoResponse {
    let brokers = state.brokers.read().await;
    let broker_list: Vec<_> = brokers.iter().map(|(id, broker)| {
        serde_json::json!({
            "id": id,
            "name": broker.name,
...
async fn initialize_market() -> AppState {
    ...
    brokers.insert("direct_access".to_string(), Broker::new(
    ...
    brokers.insert("ecn_broker".to_string(), Broker::new(
    ...
    brokers.insert("market_maker".to_string(), Broker::new(
